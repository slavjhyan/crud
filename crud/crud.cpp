...
private:
    ...
    void RestoreData()
    {
        std::ifstream data("data_count.txt");
        std::string currDataName;
        while (data >> currDataName)
        {
            userDefinedTypes.push_back(currDataName);
        }
        int size = userDefinedTypes.size();
        for(int i = 0; i < size; ++i)
        {
            std::fstream currDataFile(userDefinedTypes[i] + ".txt");
            RestoreDataFromFile(currDataFile);
        }
    }

    void RestoreDataFromFile(std::fstream& file)
    {
        std::vector<std::map<std::string, std::string>> data;
        std::map<std::string, std::string> currPair;
        std::string token;
        std::string key;
        std::string value;
        bool createExample = false;
        while(file >> token)
        {
            if(token == SEPARATOR)
            {
            ...
            {
                value = token;
                currPair.insert(std::pair<std::string, std::string>(key, value));
            }
        }
        pairsForEachDataType.push_back(data);
    }
    ...
    void CreateNewType(const std::string& newTypeName)
    {
        ...
        newFile.close();
        newFile.open("data_count.txt", std::ios::app);
        newFile << newTypeName << '\n';
    }
    std::string GetType()
    {
